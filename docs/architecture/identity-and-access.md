...
The active session supplies the account, active membership, and tenant context. Protected API handlers declare catalog permissions with `@RequirePermissions(...)`; `AuthGuard` runs before `AuthorizationGuard`. The authorization service reads current grants from PostgreSQL on each request, so membership suspension and role changes take effect without waiting for a cache timeout. Routes fail closed when identity, permission, or a supported resource scope is missing.
...
Role administration is available through `GET /api/v1/authorization/permissions`, `GET /api/v1/authorization/roles`, `POST /api/v1/authorization/roles`, `POST /api/v1/authorization/assignments`, and `DELETE /api/v1/authorization/assignments/:assignmentId`. Read routes require `authorization.roles.read`; writes require `authorization.roles.manage`. Role creation, assignment, revocation, and trusted bootstrap append security audit events in the same transaction as the mutation. The first tenant administrator is granted only after the person has an active tenant membership; use `pnpm --filter @classloom/api auth:bootstrap-admin -- <tenant-id> <email>`.
