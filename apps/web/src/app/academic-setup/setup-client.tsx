  ...
  return <div className="flex flex-col gap-6">
    ...
    {session && <>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Assign teacher</CardTitle><CardDescription>Choose an active teacher profile linked to a school account.</CardDescription></CardHeader><CardContent><FieldGroup>
          ...
          <Choice label="Teacher" value={membershipId} onChange={setMembershipId} options={staff.map((item) => ({ id: item.id, name: item.displayName ? `${item.displayName} (${item.email})` : item.email }))} placeholder="Select teacher" />
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle><Users className="inline size-4" /> Teacher assignments</CardTitle><CardDescription>{assignments.length} section subject assignments</CardDescription></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Section / subject</TableHead><TableHead>Teacher or account</TableHead></TableRow></TableHeader><TableBody>{assignments.map((item) => { const current = staff.find((member) => member.id === item.membershipId); const historical = setup?.assignmentAccounts.find((member) => member.id === item.membershipId); return <TableRow key={item.id}><TableCell>{sections.find((section) => section.id === item.sectionId)?.name ?? "Section"} / {subjects.find((subject) => subject.id === item.subjectId)?.name ?? "Subject"}</TableCell><TableCell>{current?.displayName || current?.email || historical?.displayName || historical?.email || "Former account"}</TableCell></TableRow>; })}</TableBody></Table></CardContent></Card>
      </div>
    </>}
  </div>;
}
